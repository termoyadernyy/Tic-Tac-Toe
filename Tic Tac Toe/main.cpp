...
public:
	...
	{
		for (int i = 0; i < 9; i++)
			sprite[i].setTextureRect(sf::IntRect(200 * (vid - 1), 0, 200, 200));
	}
};
...
int main() 
{
	...
	sf::Texture f;
	f.loadFromFile("assets/background.png");
	sf::Sprite fon(f);

	sf::Texture c;
	c.loadFromFile("assets/crnol.png");
	sf::Sprite vibor[2];
	for (int i = 0; i < 2; i++) 
	{
	...
	sf::Texture l;
	l.loadFromFile("assets/line.png");
	sf::Sprite line(l);
	...
	while (window.isOpen())
	{
		...
		for (int i = 0; i < 2; i++)
			if (vibor[i].getGlobalBounds().contains(pos.x, pos.y))
				vibor[i].setTextureRect(sf::IntRect(200 * i, 200, 200, 200));
			else
				vibor[i].setTextureRect(sf::IntRect(200 * i, 0, 200, 200));
		...
		for (int i = 0; i < 8; i++) 
		{
			...
			for (int j = 0; j < 2; j++)
				if (winplay[i][j]) 
				{
					...
					if (i < 3) 
					{
						line.setTextureRect(sf::IntRect(0, 0, 600, 10));
						int ly = 95 + 200 * i;
					...
					else if (i < 6) 
					{
						...
						line.setTextureRect(sf::IntRect(0, 10, 600, 600));
					else if (i == 7)
						line.setTextureRect(sf::IntRect(600, 10, -600, 600));
				}
		}
